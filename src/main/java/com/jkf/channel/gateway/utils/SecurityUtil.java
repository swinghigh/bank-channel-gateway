...
//                publicKey.getModulus().bitLength()));
    ...
    public static boolean verifyJsonStr(String result, String publicKeys) throws Exception {
        Map<String, Object> parMap = JSON.parseObject(result, new TypeReference<Map<String, Object>>(){});
        if(parMap.get(KEY_GATEWAY_SIGNATURE)==null){
            return true;
        }
        String data = preDealSignStr(parMap);
        String sign = parMap.get(KEY_GATEWAY_SIGNATURE).toString();
